...
export const VocabLibrary: React.FC = () => {
        ...
        const load = async () => {
            try {
                const [f, t] = await Promise.all([
                    firebaseService.getUserData('vocab_folders'),
                    firebaseService.getUserData('vocab_terms')
                ]);
    ...
    // --- Save Data (Hybrid) ---
    useEffect(() => {
        if (isDataLoaded && folders.length > 0) {
            firebaseService.saveUserData('vocab_folders', folders);
        }
    }, [folders, isDataLoaded]);

    useEffect(() => {
        if (isDataLoaded) {
            firebaseService.saveUserData('vocab_terms', terms);
        }
    }, [terms, isDataLoaded]);
